$(function () {
    let tanggal = sessionStorage.getItem('kepatuhanapd');
    if (tanggal) {
        let data = JSON.parse(tanggal);
        $('#tanggalAwal').val(data.tglAwal);
        $('#tanggalAkhir').val(data.tglAkhir);
    }
    tampilDataTabelKepatuhanApd();
});

$(window).on('resize', function () {
    $('#tabelKepatuhanApd').DataTable().ajax.reload(null, false);
});

function tampilDataTabelKepatuhanApd() {
    let form = $('#formFilterData');
    $('#tabelKepatuhanApd').DataTable(
        {
            bDestroy: true,
            processing: true,
            serverSide: true,
            ...
            columns: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        let html = /* html */
                            `<button data-popup='tooltip' title='Detail Kepatuhan APD' class="btn btn-info btn-sm border-0" onclick="detailKepatuhanApd(this,'${data}')">
...
function detailKepatuhanApd(el, id) {
    $.ajax({
        type: 'GET',
        url: url + '/detail/' + id,
        dataType: 'JSON',
        ...
        success: function (res) {
            ...
            if (res.sukses) {
                ...
                $('#modalDetailKepatuhanAPD').modal('show');
            }
        },
...
$('#formFilterData').on('submit', function (e) {
    ...
    if (pesanError.length > 0) {
        ...
        $(".invalid-feedback").html("");
        $('.is-invalid').removeClass('is-invalid');
        $('#tabelKepatuhanApd').DataTable().ajax.reload();
        sessionStorage.setItem('kepatuhanapd', JSON.stringify({ tglAwal: tglAwal, tglAkhir: tglAkhir }));
    }
});
                ...
                success: function (res) {
                    ...
                    if (res.sukses) {
                        swalWithBootstrapButtons.fire(
                            ...
                            res.pesan.teks,
                            "success"
                        ).then(() => $('#tabelKepatuhanApd').DataTable().ajax.reload(null, false));
                    } else {
                        swalWithBootstrapButtons.fire(
                            res.pesan.judul,
                            res.pesan.teks,
