$(function () {
    let dataStorage = sessionStorage.getItem('permintaanbmhpsteril');
    if (dataStorage) {
    ...
    tampilDataTabelPermintaanBmhpSteril();
});

function tampilDataTabelPermintaanBmhpSteril() {
    let form = $('#formFilterData');
    $('#tabelPermintaanBmhpSteril').DataTable(
        {
            bDestroy: true,
            processing: true,
            serverSide: true,
            ...
            columns: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        let html = /* html */
                            `<button data-popup='tooltip' title='Detail Permintaan BMHP/Kasa Steril' class="btn btn-info btn-sm border-0" onclick="detailPermintaanBmhpSteril(this,'${data}')">
...
function detailPermintaanBmhpSteril(el, id) {
    $.ajax({
        type: 'GET',
        url: url + '/detail/' + id,
        dataType: 'JSON',
        ...
        success: function (res) {
            ...
            if (res.sukses) {
                ...
                $('#modalDetailPermintaanBmhpSteril').modal('show');
            }
        },
...
$('#formFilterData').on('submit', function (e) {
    ...
    if (pesanError.length > 0) {
        ...
        $(".invalid-feedback").html("");
        $('.is-invalid').removeClass('is-invalid');
        $('#tabelPermintaanBmhpSteril').DataTable().ajax.reload();
        sessionStorage.setItem('permintaanbmhpsteril',
            JSON.stringify({
                tglAwal: tglAwal,
                tglAkhir: tglAkhir,
                ruangan: $('#ruangan').val()
...
$('#tabelPermintaanBmhpSteril tbody').on("click", ".formHapus button[type='submit']", function (e) {
    e.preventDefault();
    swalDelete.fire({
        title: 'Hapus Permintaan BMHP/Kasa Steril',
                ...
                success: function (res) {
                    ...
                    if (res.sukses) {
                        swalWithBootstrapButtons.fire(
                            ...
                            res.pesan.teks,
                            "success"
                        ).then(() => $('#tabelPermintaanBmhpSteril').DataTable().ajax.reload(null, false));
                    } else {
                        swalWithBootstrapButtons.fire(
                            res.pesan.judul,
                            res.pesan.teks,
