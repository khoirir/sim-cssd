$(function () {
    let tanggal = sessionStorage.getItem('monitoringmesineog');
    if (tanggal) {
        let data = JSON.parse(tanggal);
        $('#tanggalAwal').val(data.tglAwal);
        $('#tanggalAkhir').val(data.tglAkhir);
    }
    tampilDataTabelMonitoringMesinEog();
});

function tampilDataTabelMonitoringMesinEog() {
    let form = $('#formFilterData');
    $('#tabelMonitoringMesinEog').DataTable(
        {
            bDestroy: true,
            processing: true,
            serverSide: true,
...
function detailMonitoringMesinEog(el,id) {
    $.ajax({
        type: 'GET',
        url: url + '/detail/' + id,
        dataType: 'JSON',
        ...
        success: function (res) {
            ...
            if (res.sukses) {
                ...
                $('#modalDetailMonitoringMesinEog').modal('show');
            }
        },
...
$('#formFilterData').on('submit', function (e) {
    ...
    if (pesanError.length > 0) {
        ...
        $(".invalid-feedback").html("");
        $('.is-invalid').removeClass('is-invalid');
        $('#tabelMonitoringMesinEog').DataTable().ajax.reload();
        sessionStorage.setItem('monitoringmesineog', JSON.stringify({ tglAwal: tglAwal, tglAkhir: tglAkhir }));
    }
});

$('#tabelMonitoringMesinEog tbody').on("click", ".formHapus button[type='submit']", function (e) {
    e.preventDefault();
    swalDelete.fire({
        title: 'Hapus Monitoring Mesin EOG',
                ...
                success: function (res) {
                    ...
                    if (res.sukses) {
                        swalWithBootstrapButtons.fire(
                            ...
                            res.pesan.teks,
                            "success"
                        ).then(() => $('#tabelMonitoringMesinEog').DataTable().ajax.reload(null, false));
                    } else {
                        swalWithBootstrapButtons.fire(
                            res.pesan.judul,
                            res.pesan.teks,
