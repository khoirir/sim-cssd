$(function () {
    let tanggal = sessionStorage.getItem('monitoringmesinplasma');
    if (tanggal) {
        let data = JSON.parse(tanggal);
        $('#tanggalAwal').val(data.tglAwal);
        $('#tanggalAkhir').val(data.tglAkhir);
    }
    tampilDataTabelMonitoringMesinPlasma();
});

function tampilDataTabelMonitoringMesinPlasma() {
    let form = $('#formFilterData');
    $('#tabelMonitoringMesinPlasma').DataTable(
        {
            bDestroy: true,
            processing: true,
            serverSide: true,
...
function detailMonitoringMesinPlasma(el,id) {
    $.ajax({
        type: 'GET',
        url: url + '/detail/' + id,
        dataType: 'JSON',
        ...
        success: function (res) {
            ...
            if (res.sukses) {
                ...
                $('#modalDetailMonitoringMesinPlasma').modal('show');
            }
        },
...
$('#formFilterData').on('submit', function (e) {
    ...
    if (pesanError.length > 0) {
        ...
        $(".invalid-feedback").html("");
        $('.is-invalid').removeClass('is-invalid');
        $('#tabelMonitoringMesinPlasma').DataTable().ajax.reload();
        sessionStorage.setItem('monitoringmesinplasma', JSON.stringify({ tglAwal: tglAwal, tglAkhir: tglAkhir }));
    }
});

$('#tabelMonitoringMesinPlasma tbody').on("click", ".formHapus button[type='submit']", function (e) {
    e.preventDefault();
    swalDelete.fire({
        title: 'Hapus Monitoring Mesin Plasma',
                ...
                success: function (res) {
                    ...
                    if (res.sukses) {
                        swalWithBootstrapButtons.fire(
                            ...
                            res.pesan.teks,
                            "success"
                        ).then(() => $('#tabelMonitoringMesinPlasma').DataTable().ajax.reload(null, false));
                    } else {
                        swalWithBootstrapButtons.fire(
                            res.pesan.judul,
                            res.pesan.teks,
