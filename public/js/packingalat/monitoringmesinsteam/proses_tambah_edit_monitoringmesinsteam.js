...
$('#formMonitoringMesinSteam').on('submit', function (e) {
    e.preventDefault();
    ...
    $('#tabelDataAlat tbody tr').each(function (i) {
        let id = $(this).find('td:eq(3) button').attr('value');
        if (!id) {
            dataAlat.push({
    ...
    $.ajax({
        ...
        success: function (res) {
            if (!res.sukses) {
                ...
                if (res.pesan.invalidMesin) {
                    $('.form-radio-input[name="mesin"]').addClass('is-invalid');
                    $('#invalidMesin').html(res.pesan.invalidMesin);
                }
