$(function () {
    let dataStorage = sessionStorage.getItem('ujisealerpouchs');
    if (dataStorage) {
    ...
    tampilDataTabelUjiSealerPouchs();
});

function tampilDataTabelUjiSealerPouchs() {
    let form = $('#formFilterData');
    $('#tabelUjiSealerPouchs').DataTable(
        {
            bDestroy: true,
            processing: true,
            serverSide: true,
            ...
            columns: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        let html = /* html */`
                            <button type="button" class="btn btn-info btn-sm border-0" data-popup='tooltip' title='Lihat Gambar Uji Sealer' onclick="gambarUjiSealer(this,'${data}')">
...
function gambarUjiSealer(el, id) {
    $.ajax({
        type: 'GET',
        url: url + '/gambar-uji-sealer/' + id,
        dataType: 'JSON',
        beforeSend: function () {
        ...
        success: function (res) {
            ...
            if (res.sukses) {
                ...
                $('#modalGambarUjiSealer').modal('show');
            }
        },
...
$('#formFilterData').on('submit', function (e) {
    ...
    if (pesanError.length > 0) {
        ...
        $(".invalid-feedback").html("");
        $('.is-invalid').removeClass('is-invalid');
        $('#tabelUjiSealerPouchs').DataTable().ajax.reload();
        sessionStorage.setItem('ujisealerpouchs',
            JSON.stringify({
                tglAwal: tglAwal,
                tglAkhir: tglAkhir,
                hasil: $('#hasil').val()
                ...
                success: function (res) {
                    if (res.sukses) {
                        swalWithBootstrapButtons.fire(
                            ...
                            res.pesan.teks,
                            "success"
                        ).then(() => $('#tabelUjiSealerPouchs').DataTable().ajax.reload(null, false));
                    } else {
                        console.log(res.errorHapus);
                        swalWithBootstrapButtons.fire(
                            res.pesan.judul,
                            res.pesan.teks,
                            "error"
