...
<section class="content">
    <div class="container-fluid">
        <div class="col-lg-12">
            <a class="btn btn-primary mb-3" href="<?= base_url('/monitoring-mesin-eog'); ?>">
                ...
                <div class="col-lg-4">
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h3 class="card-title">Verifikasi Monitoring Mesin EOG / <?= generateNoReferensi($dataMesinEog['created_at'], $dataMesinEog['id']); ?></h3>
                        </div>
                        <div class="card-body">
                ...
                <div class="col-lg-8">
                    <div class="card card-primary card-outline">
                        ...
                        if ($dataVerifikasi) {
                            ...
                            $dataPrint = base_url('/img/monitoringmesineog/' . $dataVerifikasi['data_print']);
                            $displayPreviewIndikatorEksternal = 'block';
                            $indikatorEksternal = base_url('/img/monitoringmesineog/' . $dataVerifikasi['indikator_eksternal']);
                            $displayPreviewIndikatorInternal = 'block';
                            $indikatorInternal = base_url('/img/monitoringmesineog/' . $dataVerifikasi['indikator_internal']);
                            $displayPreviewIndikatorBiologi = 'block';
                            $indikatorBiologi = base_url('/img/monitoringmesineog/' . $dataVerifikasi['indikator_biologi']);
                            $verifikator = $dataVerifikasi['id_petugas_verifikator'];
                            $hasilVerifikasi = $dataVerifikasi['hasil_verifikasi'];
                        }
                        ?>
                        <form action="<?= base_url('/monitoring-mesin-eog/simpan-verifikasi/' . ($id ?? $dataMesinEog['id'])); ?>" method="POST" enctype="multipart/form-data" id="formVerifikasiMonitoringMesinEog">
                            <?= csrf_field(); ?>
                            <div class="card-body">
                                <fieldset class="content-group">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-4">
                                            ...
                                            <input type="hidden" name="tanggalMonitoring" value="<?= $dataMesinEog['tanggal_monitoring']; ?>">
                                            <input type="date" class="form-control" name="tanggalKeluarAlat" id="tanggalKeluarAlat" value="<?= $tanggalKeluarAlat ?? date("Y-m-d", strtotime($dataMesinEog['tanggal_monitoring'])); ?>">
                            ...
                            <?php else : ?>
                                <div class="<?= $dataVerifikasi ? 'd-flex justify-content-between' : ''; ?>">
                                    <?php if ($dataVerifikasi) : ?>
                                        <form action="<?= base_url('/monitoring-mesin-eog/hapus-verifikasi/' . $id); ?>" method="POST" id="formHapusVerifikasi">
                                            <?= csrf_field(); ?>
                                    ...
                                    <?php endif; ?>
                                    <button type="submit" class="btn btn-success float-right" form="formVerifikasiMonitoringMesinEog" id="btnSimpan">
...
<!-- /.content -->
<script src="<?= base_url(); ?>/js/packingalat/monitoringmesineog/verifikasi_monitoringmesineog.js"></script>
<?= $this->endSection(); ?>