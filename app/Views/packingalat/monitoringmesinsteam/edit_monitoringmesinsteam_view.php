...
<section class="content">
    <div class="container-fluid">
        <div class="col-lg-12">
            ...
            <form action="<?= base_url('/monitoring-mesin-steam/update/' . $dataMesinSteam['id']); ?>" method="POST" enctype="multipart/form-data" id="formMonitoringMesinSteam">
                <?= csrf_field(); ?>
                <div class="form-row">
                    <div class="col-lg-4">
                        <div class="card card-warning card-outline">
                            <div class="card-header">
                                <h3 class="card-title">Edit Monitoring Mesin Steam / <?= generateNoReferensi($dataMesinSteam['created_at'], $dataMesinSteam['id']); ?></h3>
                            </div>
                            <div class="card-body">
                                <fieldset class="content-group">
                                    <legend class="text-bold">Mesin, Operator, & Ruangan</legend>
                                    <div class="form-group">
                                        <label for="tanggalMonitoring">Tanggal</label>
                                        <input type="date" class="form-control" name="tanggalMonitoring" id="tanggalMonitoring" value="<?= date('Y-m-d', strtotime($dataMesinSteam['tanggal_monitoring'])); ?>">
                                        <div class="invalid-feedback" id="invalidTanggalMonitoring"></div>
                                    </div>
                                    <div class="form-group">
                                        <label for="jamMasukAlat">Jam Masuk Alat</label>
                                        <input type="time" class="form-control" name="jamMasukAlat" id="jamMasukAlat" value="<?= date('H:i', strtotime($dataMesinSteam['tanggal_monitoring'])); ?>">
                                        <div class="invalid-feedback" id="invalidJamMasukAlat"></div>
                                    </div>
                                    <div class="form-group">
                                        <label class="d-flex">Mesin</label>
                                        <div class="icheck-primary d-inline mr-3">
                                            <input type="radio" id="radioGetinge" value="Getinge" name="mesin" class="form-radio-input" <?= $dataMesinSteam['mesin'] === 'Getinge' ? 'checked' : ''; ?>>
                                            <label for="radioGetinge" class="form-check-label radio-label">
                                                Getinge
                                            </label>
                                        </div>
                                        <div class="icheck-primary d-inline mr-3">
                                            <input type="radio" id="radioSakura" value="Sakura" name="mesin" class="form-radio-input" <?= $dataMesinSteam['mesin'] === 'Sakura' ? 'checked' : ''; ?>>
                                            <label for="radioSakura" class="form-check-label radio-label">
                                                Sakura
                                            </label>
                                        </div>
                                        <div class="icheck-primary d-inline">
                                            <input type="radio" id="radioNouva" value="Nouva" name="mesin" class="form-radio-input" <?= $dataMesinSteam['mesin'] === 'Nouva' ? 'checked' : ''; ?>>
                                            <label for="radioNouva" class="form-check-label radio-label">
                                                Nouva
                                            </label>
                                            <div class="invalid-feedback mt-2" id="invalidMesin"></div>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="siklus">Siklus</label>
                                        <input class="form-control" name="siklus" id="siklus" type="text" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '')" title="Input harus berupa angka" value="<?= $dataMesinSteam['siklus']; ?>">
                                        <div class="invalid-feedback mt-2" id="invalidSiklus"></div>
                                    </div>
                                    <div class="form-group">
                                        <label class="d-flex mb-3">Shift</label>
                                        <div class="icheck-primary d-inline mr-3">
                                            <input type="radio" id="radioShiftPagi" value="pagi" name="shift" <?= ($dataMesinSteam['shift'] == 'pagi') ? 'checked' : ''; ?> class="form-radio-input">
                                            <label for="radioShiftPagi" class="form-check-label radio-label">
                                                Pagi
                                            </label>
                                        </div>
                                        <div class="icheck-primary d-inline">
                                            <input type="radio" id="radioShiftSore" value="sore" name="shift" <?= ($dataMesinSteam['shift'] == 'sore') ? 'checked' : ''; ?> class="form-radio-input">
